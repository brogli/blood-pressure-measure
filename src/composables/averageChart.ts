...
export function useAverageChart(config: AverageChartConfig) {
  ...
  function computeAverages() {
    ...
    const cutoff =
      selectedTimeRange.value !== null ? dayjs().subtract(selectedTimeRange.value, "month").toDate() : null;

    const filtered = cutoff ? allMeasurements.filter((m: Measurement) => m.timestamp >= cutoff) : allMeasurements;
    ...
    const averages: { date: string; systolic: number | null; diastolic: number | null; heartRate: number | null }[] = [];
  ...
  function averageField(measurements: Measurement[], field: "systolic" | "diastolic" | "heartRate"): number | null {
